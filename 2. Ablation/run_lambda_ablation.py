#!/usr/bin/env python3
"""
Lambda ablation for C-GASTON (standard InfoNCE).

Sweeps lambda (contrastive loss weight) across:
    LAMBDA_VALUES = [0.0, 0.01, 0.1, 0.5, 1.0]

Values match Technical Guide §6.4: {0, 0.01, 0.1, 0.5, 1.0}

  - lambda=0.0  : pure GASTON (no H&E contrastive loss) — internal baseline
  - lambda=0.1  : default (matches run_cgaston_pipeline.py)
  - others      : sensitivity analysis

Data loading and vision feature extraction are done ONCE before the lambda
loop to avoid redundant computation (~1h of ResNet-50 inference per run).

Run: python run_lambda_ablation.py

Outputs per lambda:
    C_GASTON_ablation_lambda/lam_{value}/{slice_id}/
        model.pt
        isodepth.npy
        labels.npy
        cgaston_best.pt
        ...
        cgaston_labels.npy
    C_GASTON_ablation_lambda/results_summary.csv   (all metrics)
    C_GASTON_ablation_lambda/aggregate_summary.csv
    C_GASTON_ablation_lambda/run_config.json
"""
...
ABLATION_DIR     = f'{BASE_DIR}/C_GASTON_ablation_lambda'
...
SUMMARY_FIELDS = ['lambda', 'slice', 'ARI', 'NMI', 'Spearman', 'MoransI', 'best_loss']
...
WARMUP_EPOCHS   = 2000
TOTAL_EPOCHS    = 10000
NUM_RESTARTS    = 5       # 5 restarts for ablation (vs 10 in main run)
LR              = 1e-3
PATCH_SIZE      = 224
...
print(f"Restarts per lambda: {NUM_RESTARTS}")
...
def build_model_payload(lam, model_state):
    return {
        'model_type': 'cgaston_lambda_ablation',
        'contrastive_mode': 'standard_infonce',
        'ablation_axis': 'lambda',
...
def write_model_outputs(save_dir, lam, model_state):
    torch.save(build_model_payload(lam, model_state), f'{save_dir}/model.pt')
...
def write_summary_files(output_dir, rows):
    ...
    for lam in LAMBDA_VALUES:
        lam_rows = [row for row in rows if row['lambda'] == lam]
        aggregate_row = {'lambda': lam}
        for metric in ['ARI', 'NMI', 'Spearman', 'MoransI', 'best_loss']:
            mean_value, std_value = population_mean_std([row[metric] for row in lam_rows])
...
# Training
# ============================================================
def train_cgaston(model, S_torch, A_torch, V_torch, lam,
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        if lam > 0 and epoch > warmup_epochs:
            ...
            z_m_b, _, _ = model.molecular_embedding(S[perm])
            z_v_b     = model.vision_embedding(V[perm])
            loss_cont = info_nce_loss(z_m_b, z_v_b, temperature)
...
# ============================================================
# STEP 1 — Load data + extract vision features ONCE
# ============================================================
print("\n" + "="*60)
...
# ============================================================
print("\n" + "="*60)
print("STEP 3 — Lambda ablation")
print("="*60)

# Accumulate all rows for CSV
csv_rows = []

# Summary: lam -> {metric: [per-slice values]}
summary = {}

for lam in LAMBDA_VALUES:
    lam_tag = f"lam_{lam:.3f}".replace('.', 'p')   # e.g. lam_0p100
    out_root = f'{ABLATION_DIR}/{lam_tag}'
    os.makedirs(out_root, exist_ok=True)

    print(f"\n{'='*60}")
    print(f"lambda = {lam}  (tag: {lam_tag})")
    print(f"{'='*60}")

    lam_aris, lam_nmis, lam_spears, lam_morans = [], [], [], []
    ...
    for sid in ALL_SLICES:
        ...
        write_model_outputs(save_dir, lam, best_model_state)
        ...
        lam_aris.append(ari); lam_nmis.append(nmi)
        lam_spears.append(sp); lam_morans.append(mi)

        print(f"  {sid}: ARI={ari:.4f} NMI={nmi:.4f} Spear={sp:.4f} Moran={mi:.4f}  "
              f"(best_recon={best_loss:.4f})")

        csv_rows.append({
            'lambda': lam, 'slice': sid,
    ...
    summary[lam] = {
        'ARI':      (np.mean(lam_aris),   np.std(lam_aris)),
        'NMI':      (np.mean(lam_nmis),   np.std(lam_nmis)),
        'Spearman': (np.mean(lam_spears), np.std(lam_spears)),
        'MoransI':  (np.mean(lam_morans), np.std(lam_morans)),
    }
...
print("\n" + "="*60)
print("LAMBDA ABLATION SUMMARY  (mean ± std over 8 slices)")
print("="*60)
print(f"{'Lambda':>8}  {'ARI':>14}  {'NMI':>14}  {'Spearman':>14}  {'Moran I':>14}")
print("-"*72)
for lam in LAMBDA_VALUES:
    s = summary[lam]
    print(f"{lam:>8.3f}  "
