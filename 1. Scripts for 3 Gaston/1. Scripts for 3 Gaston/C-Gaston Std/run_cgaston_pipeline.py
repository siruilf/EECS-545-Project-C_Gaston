#!/usr/bin/env python3
"""
Standalone C-GASTON training for all 8 DLPFC slices with pre-computed GLM-PCA.

Run: python run_cgaston_pipeline.py
"""
...
OUTPUT_DIR = f'{BASE_DIR}/C_GASTON_results'
...
def write_model_outputs(save_dir, model_state, legacy_state):
    ...
    torch.save(legacy_state, f'{save_dir}/cgaston_best.pt')
...
def train_cgaston(model, S_torch, A_torch, V_torch,
                  ...
                  lam=0.1, temperature=0.07, batch_size=256, lr=1e-3,
...
# ============================================================
print("="*60)
print("C-GASTON Standalone — all 8 DLPFC slices")
print("="*60)
...
for sid in ALL_SLICES:
    ...
    del data[sid]['patches']  # free memory
    print(f"  Vision features: {features.shape}")

# --- Step 4: Train C-GASTON ---
print("\n[Step 4] Training C-GASTON...")
all_results = {}
for sid in ALL_SLICES:
    print(f"\n{'='*60}")
    ...
    for restart in range(NUM_RESTARTS):
        ...
        mdl, hist = train_cgaston(
            ...
            lam=LAMBDA_CONTRASTIVE, temperature=TEMPERATURE,
